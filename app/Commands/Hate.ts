...
export default class FollowAge implements CommandContract {
  ...
  public commandName = 'Ódio'

  // The description of the command
  public description = 'Display how much a viewer hates the streamers'
  /**
   * Execute the command.
   *
  ...
  public async execute(client: CommandClient, channel: CommandChannel, args: CommandArguments, data: CommandData) {
    ...
    return client.say(
      channel,
      `@${displayName} odeia a @${streamer.displayName} algo equivalente a ${percentage}%. Pode ser amor reprimido 👀`,
    )
  }
}
